...
class Propose(CustomAction):
    ...
    def run(self, belief):
        ...
        self.data = [{k : v for k, v in row.items() if k in                                             \
                     self.frame.get_all_slots(exclude_fkeys=True)}
                     for row in self.data]

        if not self.data:
            print("system belief: {}".format(belief))
    ...
    def vui_dm_run(self):
        return templates.propose_run_template                                                           \
                .format(table_name=self.frame.table['table_name'],
                        ...
                        feats=self.frame.get_all_slots(exclude_fkeys=True))
...
class Multichoice(CustomAction):
    ...
    def __init__(self, frame, slot, all_slots):
        table_name = frame.table['table_name']
        table_name = "".join([x.capitalize() for x in table_name.split("_")])
    ...
    def to_natural_language(self):
        options = ", ".join(sum([list(res.values()) for res in self.data[:5]], []))
        prompt = "you can choose between: {}".format(options + (" and more" if                          \
                                                 len(self.data) > 5 else ""))
        return prompt
    ...
    def run(self, belief):
        ...
        self.data = []
        rows = db.kb_lookup(self.frame.table['table_name'],
                            belief)
        seen_values = set()
        for row in rows:
            if row[self.target] not in seen_values:
                self.data.append({self.target: row[self.target]})
                seen_values.add(row[self.target])
    ...
    def vui_dm_run(self):
        return templates.multichoice_run_template                                                       \
                .format(table_name=self.frame.table['table_name'],
                        slots=self.all_slots, target=self.target)
...
class Lookup(CustomAction):
    ...
    def to_string_verbose(self):
        return (self.name + "_" + ("noResult" if not self.data else "ok")
                          + "_with_{}".format(self.last_system_belief))
...
class PrepareForUpdates(CustomAction):
    ...
    def to_natural_language(self):
        ...
        return "so you want to modify {} with id {}".format(self.frame.entity                           \
                                .replace("_", " "), self.frame.content[pkey])
...
class ReloadOptions(CustomAction):
    ...
    def run(self):
        self.slot_events = {k: v for k, v in self.frame.history[-1]                                     \
                            .items() if self.frame.content[k] != v}
        self.frame.rewind()
        return self
...
class BeginTask(CustomAction):
    ...
    def to_natural_language(self):
        predicates = {'insert': "make", 'select': "search for",
                      'delete': "cancel",  'update': "modify" }
        
        task = "{} a {}".format(predicates[self.frame.subdialog.lower()], 
                            self.frame.entity.lower().replace("_", " "))
...
class RepeatLastTurn(CustomAction):
    ...
    def to_natural_language(self):
        return "i said: {}".format(self.last_utterance)

    def to_string_verbose(self):
        return self.name + "_{}".format([a.name for a in self.last_turn])

    def run(self, last_turn, last_utterance):
        self.last_utterance = last_utterance
        self.last_turn      = last_turn
        return self

    def vui_dm_run(self):
        return templates.repeatlastturn_run_template

class AnswerQuestion(CustomAction):

    def __init__(self, frame, slot):
        db = Database.get_instance()
        table  = "".join([x.capitalize() for x in frame.table['table_name'].split("_")])
        column = "".join([x.capitalize() for x in db.slot_to_column(slot).split("_")])
        name   = "AnswerQuestionAbout{}{}".format(table, column)
        CustomAction.__init__(self, name, frame)
        self.requested = slot

    def to_natural_language(self):
        db = Database.get_instance()
        prompt = "the {tab} [{obj}] {col} is the following: {ans}."
        column = db.slot_to_column(self.requested)
        table  = self.frame.table['table_name']

        obj = ", ".join(["{}: {}".format(db.slot_to_column(k).replace("_", " "),
            v) for k, v in db.remove_fkeys_from_result_set(self.object).items()                          \
            if k in self.frame.get_all_slots(exclude_fkeys=True)])

        return prompt.format(tab=table, obj=obj, col=column, ans=self.answer)

    def run(self, data):
        results = self.frame.get_results_to_show()
        
        if not data:
            self.object = results[0]
        
        else:
            if 'index' in data.keys():
                self.object = results[data['index'] - 1]
            else:
        ...
        self.answer      = self.object[self.requested]
        self.slot_events = {k : v for k, v in self.object.items()}
        self.frame.update(self.slot_events)

        return self

    def vui_dm_run(self):
        return templates.answerquestion_run_template                                                    \
                .format(slot=self.requested,
                        feats=self.frame.get_all_slots(exclude_fkeys=True))