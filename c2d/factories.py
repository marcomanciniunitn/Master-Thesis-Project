...
class FrameFactory:
	...
	def __build_frames(self, subtask, frames):
		...
		if subtask['operation'] == "insert":
			...
			requestables = [db.column_to_slot(s, table_name) 											\
						   for s in subtask['requestables']]
			frame = ExtendedFrame(entity, slots, subtask['frame_priority'], 
						 			priorities, dependencies, requestables, 
						 			   subtask['operation'], parent=parent)
		frames.append(frame)
...
class GoalFactory:
	...
	def __build_goals(self, subtask, goals, task_predicate_argument):
		...
		requestables = [db.column_to_slot(s, subtask['table']) for s in 								\
											   subtask['requestables']]
