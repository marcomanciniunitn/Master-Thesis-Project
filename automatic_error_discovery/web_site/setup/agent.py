...
class DM():
	...
	def run(self):
		...
		actions = []
		self.im.read_intent()
		self.im.send_actions([Greet()])

		while True:
			intent = self.im.read_intent()
			frame = None

			if not sum([f.history for f in self.frames], []):						# first turn of conversation
				root_frame = self.get_root_frame()
				actions.append(BeginTask(root_frame).run(self.frames))
			
			if isinstance(intent, ThatsIt):
				self.im.send_actions([agent_Bye(), Finalize(None).run()])
				...
				frame = self.get_frame(intent.entity)
				self.im.send_actions([Multichoice(frame, 
							  intent.slot).run(belief)])
				continue
	...
	def complete_subdialog(self, actions, frame):
		...
		if frame.subdialog.lower() == "insert":
			...
			if self._elicit_user_preference(frame, actions):
				self.im.send_actions([PrepareForUpdates(frame).run(),
				...
				while not isinstance(intent, Deny):
					...
					if isinstance(intent, Inform):
						frame.update(intent.data)
						self.im.send_actions([MoreUpdates(frame.entity)])
		...
		while 1:
			...
			if isinstance(intent, Inform):
				if 'index' in intent.data.keys():
					...
					self.im.send_actions([Propose(frame)												\
						.run(self.get_system_belief())])
				else:
					...
					if updates:
						actions.append(ReloadOptions(frame).run())
						return False

					self.im.read_intent()
					frame.update(intent.data)
					reset_dependencies = ResetDependencies(frame).run()
					lookup = Lookup(frame).run(self.get_system_belief())
					propose = Propose(frame).run(self.get_system_belief())
					self.im.send_actions([reset_dependencies, lookup, propose])
				...
				belief = self.get_system_belief()
				self.im.send_actions([a, Propose(frame).run(belief)])

			elif isinstance(intent, AskMoreOptions):
				self.im.read_intent()
				belief = self.get_system_belief()
				self.im.send_actions([LoadMoreOptions(frame).run(),
									   Propose(frame).run(belief)])

			elif isinstance(intent, AskPrevOptions):
				self.im.read_intent()
				belief = self.get_system_belief()
				self.im.send_actions([LoadPrevOptions(frame).run(),
									   Propose(frame).run(belief)])

			elif isinstance(intent, AskHeadOptions):
				self.im.read_intent()
				belief = self.get_system_belief()
				self.im.send_actions([LoadHeadOptions(frame).run(),
									   Propose(frame).run(belief)])
			else:        
				return False
...
class ExtendedFrame(Frame):
	...
	def __init__(self, entity, slots, fill_order, priorities, dependencies,
								subdialog, parent=None, results_to_show=3):
		
		Frame.__init__(self, entity, slots, fill_order, priorities, subdialog,
																parent=parent)
